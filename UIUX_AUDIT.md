...
- **~~1 monolithe POS~~ — mesuré, décision : ne pas découper.** `POS.tsx` fait **1 403 lignes**, dont **1 023 de logique** et **380 de rendu** (bloc `return`, l. 1024→fin). Les **vues lourdes sont déjà déléguées** à 8 composants (`POSProductGrid`, `POSCart`, `POSModals`, `POSSuccessModal`, `POSPaydunyaOverlay`, `POSCashierClosed`, `RefundModal`, `TicketZModal`) + `BarcodeScanner` en lazy : le « découper modales/grille » recommandé par l'audit **est donc déjà fait**. Ce qui reste dans le rendu est le châssis (wrapper plein écran, header POS) — pas des vues à extraire.
  Le poids réel est l'**orchestration** : **58 `useState`, 27 `useEffect`, 36 handlers, 11 `useRef`** — le cerveau du checkout (panier, paliers, TVA, hors-ligne, fidélité, remboursements, session de caisse), états interdépendants **sur le chemin de l'argent**. Les découper en hooks n'apporte **rien à l'utilisateur** et déplace un risque que ni `tsc` ni les unitaires n'attrapent (un effet qui part dans le mauvais hook change un ordre d'exécution, pas un type). **Ne pas le faire à l'aveugle.** Si un jour : chantier humain, rejouant checkout / paiement / hors-ligne / remboursement de bout en bout.
  *(Chiffres remesurés le 2026-07-31 — une première rédaction annonçait « rendu = 39 lignes de JSX », 10× sous la réalité, plus 60 états et 29 effets. Le fichier a déjà publié cinq mesures fausses ; celles-ci sont comptées sur le fichier, pas estimées.)*
