...
⚠️ **L'IMAGE BACKEND S'INSTALLE SANS LOCKFILE** — le contexte `apps/backend` n'en porte aucun : `RUN npm install` résout chaque plage contre le registre **DU JOUR**. Le 2026-09-12, vite 8.3.0 (publié le 10) a fait planter npm 10.9.8 sur une boucle de peers (`edgesOut` null) : déploiement échoué à `npm install`, **`package.json` identique au dernier succès**, CI unitaire VERTE (elle installe depuis le lockfile racine). Parade : `overrides.vite` EXACT dans `apps/backend/package.json` (ignoré par la racine du monorepo, appliqué dans l'image), aligné sur le lockfile par `dockerInstallPins.test.ts`. ⚠️ **Un déploiement Railway FAILED se diagnostique par les journaux de BUILD** (`railway logs --build <id>`) et se **reproduit dans `node:22-slim`** avant tout correctif — le premier correctif supposé (épingler vitest) a échoué au test. Relancer ne sert à rien sur une résolution déterministe.
