...
npm run verify:date-refusal --workspace=apps/backend   # ⚠️ PAS à chaque déploiement : APRÈS UNE MONTÉE de zod / fastify / fastify-type-provider-zod, ou du Dockerfile — là seulement la source peut rester juste et l'exécution changer. Une régression de CODE est déjà prise par `dateSchemas.test.ts` en CI (sabotage : 6 rouges). MANUEL, jamais en CI (aucun secret au dépôt)
...
⚠️ **L'IMAGE BACKEND S'INSTALLE PAR `npm ci` DEPUIS `apps/backend/package-lock.json`, DÉRIVÉ du lockfile racine** — `npm run lock:backend`, à relancer après TOUTE montée de dépendance backend. Avant le 2026-09-12 le contexte Docker n'avait aucun lockfile : `npm install` résolvait contre le registre du jour (vite 8.3.0 a planté npm et bloqué le déploiement ; l'image tournait avec twilio 6.1.1 quand la CI testait 6.0.2). La dérivation GARDE les versions de la racine — elle ne résout pas à neuf — et refuse de générer si un override de SÉCURITÉ de la racine manque au backend (`overrides` n'est honoré qu'à la racine d'une installation ; `africastalking` épingle un `axios` et un `joi` vulnérables). Verrou `dockerLockfile.test.ts`, en CI AVANT déploiement : lockfile ↔ package.json, chaque `nom@version` de l'image présent dans la racine, overrides recopiés. ⚠️ **Un déploiement Railway FAILED se lit dans les journaux de BUILD** (`railway logs --build <id>`) et se **reproduit dans `node:22-slim`** avant tout correctif — le premier correctif supposé (épingler vitest) a échoué au test.

⚠️ **`tsc --noEmit` N'INSPECTE PAS `prisma/`** — `tsconfig.json` a `include: ["src/**/*"]`. Les scripts de `apps/backend/prisma/` (seeds, `verify-*`, scripts de correction) sont donc **hors du rituel** : annoncer « tsc 0 » ne dit RIEN d'eux. Vérifié par contre-preuve le 2026-08-12 (faute de type injectée, tsc resté vert). Bonne nouvelle corollaire : le **build Docker ne les compile pas non plus**, donc un `import` d'un paquet hoisté à la racine (`playwright`) n'y casse pas le déploiement — contrairement à `src/`, cf. ci-dessus. Pour les vérifier : `npx tsc --noEmit --skipLibCheck --strict false prisma/<fichier>.ts`.
...
**Corollaire — le smoke de version ne prouve pas un déploiement.** `npm run smoke:version` compare la version, donc reste **vert quand le déploiement n'a pas eu lieu** si la version n'a pas bougé (vu 2 fois, et un commit qui ne touche qu'un test n'en bump aucune). ✅ **GARDÉ EN CI depuis le 2026-08-11** — job `deploy-landed` / `npm run verify:deploy --workspace=apps/backend`, sur `main` uniquement (une PR ne déploie rien). La preuve est l'ÂGE du conteneur : **`boot = serverTime − uptime`**, postérieur à une référence prise au début du job. ⚠️ **Les deux valeurs viennent du SERVEUR** — l'horloge du runner n'entre jamais dans la comparaison. ⚠️ **DIX déploiements Railway ont échoué en deux mois sans qu'aucun signal ne parte** : conteneur sain, `/health` à 200, puis « Stopping Container », aucune erreur dans les logs — et le MÊME commit relancé à la main réussit. La production ne tombe jamais (l'ancien conteneur reste servi), et c'est ce qui rend le silence coûteux. Le message d'échec dit de RELANCER avant de suspecter le code. Aucun secret requis : pas d'`exit 0` possible sur une configuration absente.
...
- **Couleurs** : `var(--)` systématiquement. Exceptions : palettes sémantiques, SVG des marqueurs de carte, PDF, `.public-scope`, `#fff` boutons colorés, défs thème. ⚠️ FS macOS : ne pas créer `Button.tsx`/`Tabs.tsx`/`Tooltip.tsx` (collision shadcn minuscules).
...
- **Raccourci démo — ⚠️ LE WEB ET LE MOBILE NE GARANTISSENT PAS LA MÊME CHOSE.** Web : le chunk est **absent** du `dist/`, `verify:demo-flag` le prouve sur l'artefact. Mobile (`src/lib/demoAccounts.ts`, `EXPO_PUBLIC_DEMO_MODE`) : le drapeau **masque à l'exécution** et **ne retire RIEN de l'artefact** — MESURÉ, Metro n'élimine pas la branche morte comme Rollup. **Ne pas transposer la garantie web au mobile.** Défaut ÉTEINT ; verrous `demoAccounts` (règle) et `demoAccountsRow` (rendu).
