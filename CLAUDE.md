...
- **Webhooks paiement** : signature HMAC-SHA256 du raw body (`timingSafeEqual`). Orange = fail-closed ; Wave = fail-open sans secret (cf. dette). Env : `ORANGE_MONEY_WEBHOOK_SECRET`, `WAVE_WEBHOOK_SECRET`.
...
- **Railway (backend)** : `DATABASE_URL` (prod !), `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_WHATSAPP_FROM`, `WAVE_WEBHOOK_SECRET`, `ORANGE_MONEY_WEBHOOK_SECRET` (à poser), Resend, Redis.
