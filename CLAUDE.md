...
| c18cabb | feat(receipt): **détail du paiement mixte sur le ticket** (WhatsApp + PDF) — helper `mixedSplitParts` |
| 3e8f2b5 | fix(prefs): **devise/langue survivent aux OTA & restarts** — `syncCurrencyFromTenant` attend la réhydratation du persist (race) |
| fb0a83b | feat(loyalty): **v2 remises par palier + carte fidélité numérique** (mobile) — badge POSCart, carte `LoyaltyCardDigital` |
| bdf0dd1 | fix(loyalty-card): **QR OTA-safe** — `qrcode` pur JS + `expo-print` (remplace `react-native-qrcode-svg`/`-view-shot` natifs) |
| 2ab5ca3 | feat(loyalty-card): bouton **« Voir la carte fidélité »** dans l'alerte post-vente (client lié + `enableLoyalty`) |
| 99af9cb | feat(loyalty-card): **redesign carte numérique = design web** (2 zones par palier : haute sombre badge/nom/code/points + basse fond thème progression/stats 2 colonnes) |
| 92be361 | fix(loyalty-card): **QR invisible sur device** — `toDataURL` exige un canvas absent sous Hermes → rendu en **grille de `<View>`** via `QRCode.create()` (pur JS) + **SVG inline** pour le PDF |
...
*Dernière mise à jour : 2026-06-10 — **Carte fidélité numérique : redesign « design web » + fix QR invisible sur device**. (1) `99af9cb` redesign en **2 zones par palier** (haute sombre `dark/mid/accent` Bronze `#1C1007/#3D2010/#F5A623`, Silver/Gold déclinés : badge pill + nom + code + points 28px blancs + QR cadre blanc 64×64 ; basse fond thème : progression + stats 2 colonnes « Prochain palier / Points restants » + bouton Partager) ; HTML PDF aligné. ⚠️ ces couleurs viennent du **prompt**, PAS du web actuel (`ui/LoyaltyCardDigital.tsx` web = `#3D2B0A→#1a1207`/`#CD7F32`) → si on veut l'alignement inverse, c'est un chantier repo `habashop`. (2) `92be361` **fix QR invisible** : `QRCode.toDataURL()` **rejette sous Hermes** (Metro résout `qrcode` via le champ `browser` → `CanvasRenderer` → `document.createElement('canvas')` inexistant → `'You need to specify a canvas element'`, avalé par `.catch()` → `qrUri=''` → `<Image>` vide ; le PDF était cassé pareil). **Le QR `qrcode` pur JS n'a JAMAIS marché device via `toDataURL`** (jamais vérifié device avant). **Solution OTA-safe** : `QRCode.create()` (pur JS, sans canvas) → matrice de modules rendue en **grille de `<View>`** (carte) + **SVG inline** construit de la même matrice (PDF, rendu par le webview) ; fallback texte monospace si `create()` échoue ; **plus aucun `toDataURL`**. ⚠️ **ne PAS revenir à `toDataURL`/`<Image>` base64 pour un QR** sous RN/Hermes — `create()`+grille (ou `toString({type:'svg'})` côté webview) seulement. Web inchangé (canvas dispo en navigateur, OK). **OTA `preview` runtime 1.4.2 → Update `48054237` (QR fix), précédent `8abe98ba` (redesign)** · `tsc` 0 · **119 tests verts** ; pas de nouveau build natif. — Précédent : 2026-06-06 — **Loyalty v2** (remises par palier + carte fidélité numérique) sur mobile + backend/web (repo `habashop` `73d4bb7e`). Mobile : `fb0a83b` (v2 + carte), `bdf0dd1` (QR OTA-safe `qrcode`+`expo-print`, retrait deps natives `react-native-svg`/`-qrcode-svg`/`-view-shot`), `2ab5ca3` (« Voir la carte » post-vente). ⚠️ remise auto = **calcul serveur** (plafond 50 %), mobile = affichage seul. **OTA `preview` runtime 1.4.2 → Update `019e9a70`** · `tsc` 0 · **119 tests verts**. Voir **Session 2026-06-04/05 §9** ci-dessus. — Précédent : 2026-06-05 — **session remboursement + fidélité + robustesse + e2e + facture PDF + paiement mixte (+ détail reçu) + fix persistance devise/langue** (Update `019e9812` ; **119 tests verts** ; +`5cae7f4` Facture PDF, +`8b2105f` paiement mixte, +`c18cabb` détail mixte ticket, +`3e8f2b5` fix persistance prefs) — précédent Update `019e95b7` ; pas de nouveau build natif). Livré : (1) **annulation/remboursement** de vente (`a50f820`) + fix « Rembourser » rendu inline dans la liste (`d7eecc9`) ; (2) **fidélité** — feedback « +N pts » + carte honnête (`920bb88`), **seuils/taux configurables par tenant** (`d4a260a`) ; (3) **robustesse P1-A/B/C** — Error Boundaries par route + ventes résilientes idempotentes retry→file (`468934d`) ; (4) **smoke Maestro** e2e (`07e1b31`, non lancé). **101 tests verts**, `tsc` 0. Sentry : unique crash = Fabric natif `addViewAt` (stale, 3 juin) ; **web sans Sentry** (angle mort) ; `SENTRY_AUTH_TOKEN` tourné (`.env` + EAS). Voir **Session 2026-06-04/05** ci-dessus. — Précédent : **APK 1.4.2 buildé** (build `33940b3d`) : fix du **label version hardcodé** « 1.2.0 » en Réglages → lit désormais `Constants.expoConfig?.version` (`dace38c`). Le label trompeur faisait croire que le crash Fabric Caisse persistait alors qu'il est corrigé depuis 1.4.0 (`d949c78`). `newArchEnabled` reste absent (Fabric ON ; `false` casse le build — reanimated 4). À valider device : désinstaller l'ancienne app puis vérifier « 1.4.2 » affiché + bouton Caisse OK. Précédent : (1) scanner code-barres durci + Caisse scan seul ; (2) pack robustesse : Error Boundary, 8 catch vides comblés, reçu PDF (expo-print, validé device) ; (3) pack build : expo-background-task (remplace background-fetch déprécié) + crash reporter Sentry (guardé Expo Go/DSN) ; (4) parité kiosque TVA/client/remise ; (5) **APK 1.4.1** (`b999af75`) Sentry complet (DSN + source maps). Audit interne au repo = soldé.*
...
- Nouveaux fichiers : `src/services/invoicePdf.ts` · `src/components/sales/InvoiceButton.tsx` · `src/lib/paymentSplit.ts` + tests `invoicePdf`/`paymentSplit`.

### 7. Détail du paiement mixte sur le ticket (`c18cabb`)
- Lève la limite ci-dessus : le reçu **WhatsApp ET PDF** détaille désormais le mixte.
- `TicketOptions` gagne `split?: MixedSplit` ; helper **exporté `mixedSplitParts()`** (`whatsappTicket.ts`, libellés fr/en/es/it des seaux **non nuls**) réutilisé par `printReceipt`. Si `paymentMode==='mixed'` → ligne « Mixte » + sous-lignes par méthode (Espèces / Mobile Money / Carte) + montants.
- POS `onSuccess` : mode + split lus depuis le **payload envoyé** (`variables` de la mutation), car le mode `'mixed'` n'est jamais écrit dans `posStore`. Historique : `saleTicket()` lit la ventilation depuis `SaleRecord` (`cashAmount/mobileMoneyAmount/cardAmount`, renvoyés par `GET /api/sales`).
- Tests `ticketSplit` (mixedSplitParts + rendu mixte/simple). **114 tests verts**.

### 8. Persistance devise/langue — fix race hydration (`3e8f2b5`)
- **Symptôme** : à chaque OTA/redémarrage, devise (et langue) « sautaient » (devise écrasée par le tenant). **Cause** : le store EST persisté (clé AsyncStorage **`habashop-settings`**, `partialize` lang/currency/currencyManuallySet/theme/…), mais `syncCurrencyFromTenant` (authStore, appelé au boot par `restoreSession`/`setAuth`) lisait `useAppStore.getState()` **AVANT** la fin de la réhydratation → `currencyManuallySet` valait son état initial `false` → choix manuel écrasé. (Langue : sans surcharge, dépendait de la même fenêtre ; aucun code de locale device n'existe.)
- **Fix** : `appStore.whenAppStoreHydrated()` (via `persist.hasHydrated()`/`onFinishHydration()`) ; `syncCurrencyFromTenant` devient **async** et **attend** la réhydratation avant de lire `currencyManuallySet`. Règle pure `src/lib/prefs.ts` `shouldApplyTenantCurrency` (tenant appliqué **uniquement** si pas de choix manuel). Prefs **non effacées au logout** (déjà le cas). Langue déjà persistée → conservée.
- ⚠️ Tout code qui applique une **valeur par défaut** au démarrage (devise tenant, etc.) doit d'abord `await whenAppStoreHydrated()` — sinon il lit l'état initial et écrase les prefs persistées.
- Nouveaux fichiers : `src/lib/prefs.ts` + test `appPrefs` (règle pure + persistance write/rehydrate). **119 tests verts**.

### 9. Loyalty v2 — remises par palier + carte fidélité numérique (`fb0a83b`, `bdf0dd1`, `2ab5ca3`)
> Backend + web livrés en parallèle (repo `habashop`, commit `73d4bb7e`) : migration additive `bronzeDiscount/silverDiscount/goldDiscount` (Tenant, défauts 5/10/15 %) + `loyaltyDiscount` (Sale) ; `POST /api/sales` applique la remise auto du palier client (plafond **50 %** anti-abus) ; `GET /api/customers/:id/loyalty-card` (scope tenant strict).

- **Remise par palier (affichage)** : `src/lib/loyalty.ts` → `discountForTierDisplay(tier, bronze, silver, gold)` (0 = non configuré). Le **calcul réel reste 100 % serveur** ; le mobile n'affiche que la valeur.
- **POSCart** : badge ⭐ « Remise fidélité X% appliquée » si client lié + `tenant.enableLoyalty` + remise > 0 (lue depuis le store tenant + `loyaltyPoints` client).
- **LoyaltyCard** (fiche client) : remplace « Récompenses & remises : à venir » par « Remise X% sur vos achats » quand la remise du palier > 0 ; sinon garde « à venir ».
- **Fiche vente** (`sales/index.tsx`) : ligne `⭐ Remise fidélité − X` dans le récap si `sale.loyaltyDiscount > 0`.
- **Carte fidélité numérique** : `src/components/customers/LoyaltyCardDigital.tsx` (carte tier-colorée 340×200, points, progression, boutique). Ouverte depuis : fiche client (bouton « 🎴 Partager la carte ») **et** l'alerte post-vente (« 🎴 Voir la carte fidélité » si client lié + `enableLoyalty`). Montée **on-demand** (anti-crash Fabric).
- ⚠️ **OTA-safe (piège résolu)** : `react-native-qrcode-svg` + `react-native-view-shot` exigent **`react-native-svg`** (module **natif absent** → export OTA cassé). Remplacés par **`qrcode` (pur JS)** → `toDataURL` PNG rendu en `<Image>`, et **`expo-print`** (déjà présent) pour générer/partager la carte en PDF. **Ne PAS** réintroduire ces deps natives sans nouveau build EAS.
- types : `Tenant`/`LoyaltyResponse` gagnent `bronze/silver/goldDiscount` ; `SaleRecord` gagne `loyaltyDiscount` ; nouveau `LoyaltyCardData`. `customersApi.loyaltyCard(id)`. i18n fr/en/es/it. **119 tests verts**.
...
- **Loyalty v2** : activer `enableLoyalty` + remises côté web → vente avec client qualifié → badge remise dans le panier, `loyaltyDiscount` sur la vente, remise affichée dans LoyaltyCard ; carte numérique (QR + partage PDF) depuis fiche client et après-vente.
