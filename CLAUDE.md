...
### Tests
- **Front : 357 vitest / 44 fichiers** (helpers purs + anchor tests + contraste AA sur les 2 thèmes concrets dark+light). Lancer **`vitest run` COMPLET** avant tout push touchant landing/login/thème (`landing.anchor.test.tsx` fige le H1 du hero). **Back : 525 vitest** (prisma mocké `vi.mock('../db')`, routes via `app.inject()`, mock `authenticate` via `vi.hoisted`). **Remise fidélité = cas PARTAGÉS backend↔mobile** : `docs/shared-fixtures/loyalty-discount-cases.json` lu par `loyaltyDiscountShared.test.ts` des DEUX côtés (anti-dérive de `computeLoyaltyDiscount` : arrondi/plafond 50 %/remise manuelle) — modifier la règle d'un côté sans l'autre fait échouer le test. OCR : `vi.hoisted()` + classe constructeur. **PDF pdfkit non grep-able** (buffer binaire) → tester présence/absence de texte en **mockant pdfkit** et capturant les `.text()` (cf. `invoiceBilledTo.test.ts`). ⚠️ Route avec `schema` zod → `app.setValidatorCompiler(validatorCompiler)` avant `register` (cf. § Sécurité). Isolation cross-tenant : `tenantIsolation.test.ts` (mock Prisma tenant-aware).
