...
- **`apps/backend`** — Fastify + Prisma + PostgreSQL (Railway), bcryptjs + JWT, Resend (emails), pdfkit (factures + Ticket Z, PDF serveur), twilio (WhatsApp auto + manuel), Sentry, crons `setInterval` natifs, `@anthropic-ai/sdk ^0.96.0` (OCR factures Vision), `@fastify/multipart` (upload fichiers).
...
- **POS/Ventes** : caisse, panier, paiements cash/wave/orange/mtn/card, remises, fidélité. **Remboursement TOTAL** (`POST /api/sales/:id/refund`, manager/admin, motif requis, restock optionnel, idempotent 409, status `refunded` exclu du CA partout, retire les points). **Idempotence création** (`idempotencyKey` body ou header `Idempotency-Key`, `@@unique([tenantId,idempotencyKey])`, course P2002 gérée). **Paiement mixte** : `Sale.cashAmount/mobileMoneyAmount/cardAmount`, validation `|somme−total|≤1` + ≥2 modes (`MIXED_SUM_MISMATCH`/`MIXED_NEEDS_TWO`), `paymentMode='mixed'` ; simple → ventilé dans le bon seau. Helper `lib/paymentSplit.ts`. Champ « Montant reçu » : négatif interdit (clamp), états rouge insuffisant / vert + monnaie à rendre.
- **Anti-survente** : `POST /api/sales` refuse `400 INSUFFICIENT_STOCK` si `stockQty < qty` (garde AVANT la transaction ; produit hors-catalogue/map = toléré). Décrément stock **atomique dans la transaction** (`tx.product.update decrement`). Front : `confirmSale` **surface l'erreur et avorte** (plus de `catch{}` muet) + **refetch `productsApi.list()` après vente** (sinon stock affiché périmé alors que le backend l'a bien décrémenté). **Tuile produit en rupture** (`stock<=0` ET `posShowStockOnTile`) : clic/clavier gardés, grisée (`opacity .45`), pastille → « Rupture ». 4 tests `overselling.test.ts`.
- **Session caisse** : `cashierIsOpen = requireCashier ? cashierOpen : !cashierForcedClosed` — **sélecteur partagé `useCashierIsOpen()`** (appStore), à utiliser PARTOUT (POS gate, Sidebar badge) ; `cashierOpen` n'est PAS dérivé seul. Persistance : `cashierOpen/Fund/Tx/CA` + `cart` **exclus** de partialize (session) ; `cashierForcedClosed` **persisté** (un refresh conserve l'état ouvert/fermé). `closeCashier()` (login/logout/fermeture) → `cashierForcedClosed=true` ; `openCashier()` → false + vide `cart`. PAS d'auto-open : après reconnexion la caisse repart fermée → bouton « Ouvrir » (sans fond si `requireCashier=false`, cérémonie+fond sinon). `requireCashier` refetché du backend au montage POS (`tenantApi.get`, loader `settingsLoaded`) car la valeur persistée localStorage peut être stale. Montants caisse stockés en **XOF** → affichés via `fmt()` (convertit), montant compté saisi en devise → `toXOF()` avant écart. ⚠️ `onClick={() => confirmSale()}` (jamais `onClick={confirmSale}` → l'event SyntheticEvent partirait dans le payload = JSON circulaire HTMLButtonElement). Toggle WhatsApp = `autoWhatsApp` tenant à chaque ouverture du modal.
...
- **PayDunya** (Sénégal & UEMOA — Wave SN, Orange Money SN, Free Money, Expresso, Djamo, Visa/MC) : `services/paydunya.ts` (fetch natif, pas de SDK). URL = **`https://app.paydunya.com/{sandbox-api|api}/v1`** (PAS `sandbox-api.paydunya.com`) selon `PAYDUNYA_MODE` (test/live). Headers `PAYDUNYA-MASTER-KEY`/`-PRIVATE-KEY`/`-TOKEN`. `createInvoice` (`checkout-invoice/create`, succès `response_code:'00'` → `token` + `response_text`=URL hébergée), `confirmInvoice` (`checkout-invoice/confirm/:token` → `status`), `verifyIpnHash` (**SHA-512(MASTER_KEY)** hex, `timingSafeEqual`, **fail-closed**). Routes (`routes/paydunyaPayment.ts`) : `GET /config` (configured/mode/methods), `POST /initiate` (CASHIER+, 503 si non configuré), `POST /status` + `GET /status/:token` (polling, auto-success `PAYDUNYA_SANDBOX_AUTO_SUCCESS=1` gated, jamais en live), **`POST /ipn`** (public, hash fail-closed→401, **réconciliation only** — pas de décrément stock dupliqué). Parser urlencoded ajouté dans `server.ts` (IPN `data[...]` à plat). `Sale.paydunyaReference String?`. **Pattern = front-polling crée la vente** (comme MTN/Campay) ; le webhook ne crée rien. Tests : `paydunya.test.ts` + `paydunya.routes.test.ts` (16). Page Intégrations : carte PayDunya = état réel via `GET /config` (badge Sandbox orange / Actif vert, méthodes réelles `config.methods`, lien « Activer la production » visible si `mode='test'`) + **stats temps réel** via `today-stats.paydunya` (tx/montant/dernière réussie, comme MTN/Campay). **Flux POS** : `POSPaydunyaOverlay.tsx` (QR noir/blanc + lien + polling 3s × 100 = 5 min) ; `POSCart` → `isPaydunyaMode = paydunyaOk && (wave||orange)` affiche un **bouton unique « Confirmer »** (déclenche `startPaydunyaPayment`), « Encaisser » masqué ; succès → `confirmSale(…, paydunyaRef)`.
- **Stats paiement du jour** (`GET /api/payments/today-stats`, lecture tout-membre, scope tenant) : count + montant XOF + dernière réussite **par fournisseur** (`mtn`/`campay`/`paydunya`, filtre par `*Reference`), jour **UTC** (même borne que Ticket Z), refunded exclus. Helper pur `computePaymentStats` (6 tests, helper `add()` factorisé). Consommé par la page Intégrations (cartes MTN/Campay/**PayDunya**, même rendu `tx`) ; le front convertit XOF→devise via `fmt`. ⚠️ Ajouter un fournisseur = étendre CE helper + `today-stats` (pas `/config`).
- **Page Intégrations** (`pages/Integrations.tsx`, route `/app/integrations`, **AdminOnly**) : cartes groupées par catégorie (Paiements / Notifications / Base de données / Hébergement / Monitoring & IA). Métriques **réelles uniquement** — latence + taux d'erreur dérivés du **ping HEAD live** (`mode:'no-cors'`), transactions du jour depuis `today-stats`. Les services sans endpoint testable (`noPing`: paiements, Redis) **masquent** la grille latence/erreurs au lieu d'afficher du faux. ⚠️ **Ne PAS réintroduire** de `calls/mois`/`uptime`/« il y a X min » hardcodés (purgés — c'étaient des valeurs inventées du design d'origine). Carte **PayDunya** : statut connecté/déconnecté + sandbox/production **réel** via `GET /api/payments/paydunya/config` (backend branché). Modal config : champs clés API en état React **éphémère** (JAMAIS de secret en localStorage — XSS).
...
- **Clients** : CRUD, Google Maps (`VITE_GOOGLE_MAPS_KEY`), loyalty, revenus cumulés. **Fournisseurs** : CRUD, soft-delete restorable. **Commandes (PurchaseOrder = fournisseurs)** : CRUD, statuts. **OCR scan factures** : `POST /api/suppliers/scan-invoice` (MANAGER+, multipart `invoice`, JPEG/PNG/PDF, 10 MB max, env `ANTHROPIC_API_KEY`) → `services/invoiceOcr.ts` — appel Claude Sonnet 4.6 Vision, fail-silent parse error, strip markdown. Front (`NewOrderModal.tsx`, mode fournisseur) : bouton scan, preview image, auto-fill articles/fournisseur/notes. ⚠️ Les `unitPrice` OCR sont **dans la devise de la facture** (pas XOF) → total affiché via `formatInCurrency(sum, currency)` et **non** `fmt(sum)` (sinon double conversion XOF). `suppliersApi.scanInvoice(file)` = fetch brut + FormData (pas d'`api.post` pour éviter le header `Content-Type` JSON).
...
### Tests
- **405 vitest front / 381 back** (vitest 4 ; helpers purs + anchor tests de découpe + contraste AA 9 thèmes). Tests backend = **prisma mocké** (`vi.mock('../db')`) — jamais la DB prod. Routes testées via `Fastify().register(routes)` + `app.inject()` en mockant `../middleware/authenticate`. OCR : 7 tests `invoiceOcr.test.ts` (mock `@anthropic-ai/sdk` via `vi.hoisted()` + classe constructeur — les arrow functions ne peuvent pas être des constructeurs).
...
- **PayDunya go-live** : actuellement `PAYDUNYA_MODE=test` (sandbox). Pour la prod : `PAYDUNYA_MODE=live` + clés live (`PAYDUNYA_MASTER_KEY`/`PRIVATE_KEY`/`PUBLIC_KEY`/`TOKEN`). ⚠️ **Flux POS non validé en runtime bout-en-bout** (clic caissier → page hébergée → polling completed) — à tester en sandbox (vérifier que `response_text` de `create` est bien une URL scannable) avant go-live. L'IPN est en réconciliation seule (la vente est créée par le front au polling). **S**
...
- ⚠️ Données prod du tenant démo (`demo-tenant-001`) hors seed : `currency='EUR'` (pas XOF malgré « Dakar »), `requireCashier=false`, `ownerPhone='+221771234567'`. `demo-tenant-002` (Abidjan) + `cmpj8s2ze0007xmtxyjsvs665` (NELSON VENTE) ont aussi un `ownerPhone`. Si tu reseedes et que le seed force `requireCashier=true`, repasse-le à `false` (sinon « Caisse fermée » au refresh sur la démo).
...
- **Google Maps `setOptions({ styles })` non fiable pour changer les tuiles** à chaud : l'API accepte l'appel mais ne force pas le re-rendu dans tous les navigateurs. Pattern correct : compteur `mapVersion` (state), effet `[theme]` qui nettoie l'ancienne instance (markers/heatmap/infowin, `mapObj.current = null`, `setMapReady(false)`, `setMapVersion(v+1)`), effet d'init avec deps `[mapsLoaded, mapVersion]`, `key={mapVersion}` sur le div carte (nœud DOM frais). Cf. `CustomerMap.tsx`.
- **Thème light vs soleil** : `soleil` est un 2ᵉ thème clair mais son nom ne contient pas `'light'`. Toujours utiliser `IS_LIGHT_THEME = (t) => t === 'light' || t === 'soleil'` quand on branch sur le thème pour piloter des styles externes (Google Maps, ombres hardcodées…). Dans `customersShared.tsx` : `MAP_BG` et `getMapStyle` utilisent ce helper.
...
- **Railway (backend)** : `DATABASE_URL` (prod !), `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_WHATSAPP_FROM`, `WAVE_WEBHOOK_SECRET`, Resend, Redis.
  ...
  - PayDunya (Wave/Orange Money SN & UEMOA) : `PAYDUNYA_MASTER_KEY`, `PAYDUNYA_PRIVATE_KEY`, `PAYDUNYA_PUBLIC_KEY`, `PAYDUNYA_TOKEN`, `PAYDUNYA_MODE` (test/live), `PAYDUNYA_SANDBOX_AUTO_SUCCESS` (=1 sandbox uniquement). Les 5 premières présentes en prod (mode test).
...
i18n complété en 8 lots (fr/en/es/it partout). Découpes/refontes/design system « Daylight » (3 vagues : lisibilité AA, 6 primitives ui/, tokens + thème soleil ☀️ haut-contraste 9ᵉ thème) livrés. Thème par défaut `gold` (violet verrouillé + or `--acc2`). 2026-06 : redesign visuel des 10 pages (pills statut, `.data-table`, cards bg2, KPI sans glow) ; typo → tokens `--fw-*` (266 occ.) ; upgrade vite 8/vitest 4 (npm audit 0) ; precache PWA −39 % + warmup routes critiques ; chantier a11y complet audité (UI/UX 93,7 / a11y 91 — boucle audit→fix→re-audit, rapports `UIUX_AUDIT_COMPOSANTS.md` + transcripts) ; intégrations MTN MoMo + Campay/Orange Money Cameroun (USSD push, polling, sandbox simulation gated, sécurité PII + fail-open) ; Campay carte Visa/MC (lien hébergé QR) ; page Intégrations enrichie (catégories, +Sentry/Redis, pills méthodes, modal PayDunya sans secret localStorage) + endpoint `today-stats` (transactions paiement du jour, données réelles, faux chiffres décoratifs purgés) ; **OCR scan factures fournisseurs** (Claude Sonnet 4.6 Vision, `services/invoiceOcr.ts`, `@fastify/multipart`, 7 tests, auto-fill NewOrderModal) ; **fixes thème light/soleil** (ombres hardcodées → `var(--sh-*)`, Google Maps re-init via `mapVersion`, `IS_LIGHT_THEME` helper) ; **npm audit 0 vuln** (esbuild 0.21.5→0.28.0, vite 5.4.21→8.0.16 dédupliqué root) ; **session de durcissement POS** (anti-survente backend `400 INSUFFICIENT_STOCK` + tuile rupture grisée + refetch stock post-vente ; refonte modèle session caisse `cashierForcedClosed` persisté + sélecteur `useCashierIsOpen` + `requireCashier` refetché ; fix double-conversion devise fermeture caisse ; fix payload JSON circulaire `confirmSale` ; data prod démo `requireCashier=false`/`currency=EUR`, `ownerPhone` posé sur 3 tenants) ; **intégration PayDunya** (Sénégal/UEMOA — `services/paydunya.ts`, routes init/status/config/IPN SHA-512 fail-closed, `Sale.paydunyaReference` via db push, flux POS `POSPaydunyaOverlay` QR+polling Wave/OM, carte Intégrations réelle, 16 tests ; flux POS non validé runtime — cf. dette). Détails : `git log`, `I18N_AUDIT.md`.
