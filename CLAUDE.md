...
- **Présences (Attendance) — couche backend Phase 1** (`c1dd19b9`) : modèle Prisma `Attendance` (`date` String "YYYY-MM-DD", `status` String `PRESENT|LATE|ABSENT|LEAVE|REST` validé route, `@@unique([tenantId,employeeId,date])`) + routes `/api/attendance` (GET ?month=YYYY-MM lecture tout-membre ; POST upsert / PATCH / DELETE écriture **ADMIN/SUPER_ADMIN/MANAGER/HR** → 403 sinon ; isolation tenant stricte). Migration `20260531150000_add_attendance` **additive**, appliquée prod via `migrate deploy`, **déployée**. ⚠️ **Frontend PAS encore câblé** : `HRAttendanceTab` reste sur localStorage `habashop_attendance` (clé `${empId}_${date}`, même format de date). **Phase 2** = brancher l'UI sur l'API + migrer localStorage→DB + décider du sort du code orphelin `POINTAGE`/`calcHeures`/`calcPonctualite` (statique, non branché — soit relier aux vraies données, soit supprimer).
...
### Tests
- **242 tests vitest front** / 176 back (currency, pagination, components UI, anchors de découpe, logique métier pure : paie/CNSS/IRPP, dates/présences HR, libellés HR, conversion EUR/USD du rapport comptable, libellés catégories Stock, matrice permissions/escalade Users, lookup Open Food Facts, hook couleur thème ; back : route lang/devise ADMIN-only, CRUD présences)
