...
- **Couleurs hardcodées → `var(--*)`** (migration **progressive**, cohérence multi-thème). **L** — ⏳ EN COURS.
  ...
  - **Constat** : après lots 1-2, la casse de thème majeure (surfaces navy + bordures de champ invisibles) est traitée. Le reste des `rgba(255,255,255,…)` in-app (~50, dans `NewOrderModal`/`UserCard`/`Pagination`/`BulletinModal`/`Header`/`PlanningGrid`…) sont surtout des **bordures/survols fins** — à convertir par petits lots, en vérifiant à chaque fois que la valeur n'est pas un blanc **intentionnel sur surface colorée/gradient** (ex. header violet).
  - **Reste** : ces ~50 rgba blancs fins + cas borderline `UpgradePlan` (page dark autonome).
