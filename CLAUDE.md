...
### Tests
- **405 vitest front / 339 back** (vitest 4 ; helpers purs + anchor tests de découpe + contraste AA 9 thèmes). Tests backend = **prisma mocké** (`vi.mock('../db')`) — jamais la DB prod. Routes testées via `Fastify().register(routes)` + `app.inject()` en mockant `../middleware/authenticate`.
