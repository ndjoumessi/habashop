...
**Backend Railway** — service `habashop`, projet **`habashop`** (⚠️ ce fichier a longtemps écrit `grateful-happiness` : ce projet **n'existe plus** dans le compte — vérifié par `railway list` le 2026-08-12, il a été renommé. Un nom de projet périmé envoie chercher là où il n'y a rien). **Auto-deploy GitHub sur push `main`.** Après CHAQUE déploiement backend, DEUX contrôles, pas un : `npm run smoke:version --workspace=apps/backend` (le `/health` DÉPLOYÉ renvoie la version racine — cf. § Versionnage) **puis** `npm run verify:date-refusal --workspace=apps/backend` (l'API déployée refuse une date invalide en 400 et n'écrit RIEN). ⚠️ Le second est **MANUEL et le restera** : il exige `VERIFY_EMAIL`/`VERIFY_PASSWORD` (compte de démonstration, cf. § Comptes démo) or le dépôt n'a AUCUN secret — le brancher en CI ferait un job qui s'ignore ou qui échoue toujours, soit le `notify-failure` sortant en `exit 0`. Il ne se déclenche pas tout seul : c'est une étape du rituel, à lancer.
