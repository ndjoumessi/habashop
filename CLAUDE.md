...
- **Garde** : méta-tests `versionSource.test.ts` (front + back + **mobile**) — échouent si un semver entre guillemets réapparaît dans `src/` (même principe que le méta-test quiet zones), **commentaires inclus** (ne pas citer un semver entre guillemets dans un commentaire). Un repli non-semver (`'0.0.0-unknown'`, `'unknown'`) est toléré.
