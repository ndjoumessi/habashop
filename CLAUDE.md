...
- **Facture PDF** ⚠️ : **DEUX générateurs vivants** — backend pdfkit (`lib/invoicePdf.ts`, LA vraie facture) et devis frontend (`utils/export.ts generateInvoice`). **Corriger les DEUX.** Tout montant imprimé passe par `pdfSafeSpaces()` / `printableAmount()` (U+202F absent de WinAnsi → « 8 /500 »), toute donnée dynamique par **`escHtml` de `lib/html.ts`** — ⚠️ **RÈGLE CANONIQUE UNIQUE, 3 jumeaux** (back/front/mobile), cas partagés `html-escape-cases.json`. Elle était **écrite ici et applicable NULLE PART** (`escHtml` en `function` LOCALE, non exportée) : mesuré le 2026-08-09, **SEPT** copies indépendantes, dont une **divergente** — `CustomerMap.tsx` couvrait `& < > "` mais **pas l'apostrophe**, celle qui sort d'un attribut en guillemets simples — et `routes/export.ts` (rapport mensuel téléchargeable) n'échappait **RIEN**. Exactement `sanitizeCsv` avant #173. ⚠️ **`&#39;`, jamais `&apos;`** ; `utils/xlsxWriter.ts` émet `&apos;` et c'est CORRECT chez lui (OOXML, pas HTML) — **exempté par raison nommée**, ne pas le fondre. ⚠️ Le méta-test prouve qu'on ne RÉÉCRIT pas la règle ; il ne dit rien de qui l'APPELLE — un fichier qui n'échappe pas ne ressemble à rien et échappe à tous les scans. D'où `exportHtmlEscape.test.ts`, sur les octets rendus. Verrous : `htmlEscapeShared` ×3 (23 chacun) · `htmlEscapeConvention` (périmètre DÉRIVÉ) · `exportHtmlEscape` — 4 sabotages.
