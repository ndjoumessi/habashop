...
- **Couleurs hardcodées → `var(--*)`** (migration **progressive**, cohérence multi-thème). **L** — ⏳ EN COURS.
  ...
  - **Constat** : après lots 1-4, toute la casse de thème majeure + le gros des bordures/survols Settings sont traités. Reste ~25 `rgba(255,255,255,…)` in-app (`NewOrderModal`/`BulletinModal`/`PlanningGrid`/`Goals`/`Forecasts`/`MarkdownRenderer`/`CustomerMap`…) — bordures/survols fins épars, à finir par petits lots en gardant le **blanc intentionnel sur surface colorée** (ex. header violet `BulletinModal`).
  - **Reste** : ces ~25 rgba blancs fins + cas borderline `UpgradePlan` (page dark autonome).
