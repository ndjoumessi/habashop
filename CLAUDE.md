...
### Tests
- **200 tests vitest front** / 150 back (currency, pagination, components UI, anchors de découpe, logique métier pure : paie/CNSS/IRPP, dates/présences HR, conversion EUR/USD du rapport comptable)
...
- **Tests de logique métier** : ⏳ **bien couvert** (`3b3f038e`→`4e9f4678`, **175 front** / 145 back). Fonctions **pures** testées : `pricing.test.ts` (paliers POS `resolveTierPrice`/`validatePriceTiers`), `rbac.test.ts` (`canAccess`/`getLandingForRole`), `payroll-calc.test.ts` (`calcNet`/`calcBrut` + **`payrollBreakdown` CNSS/IRPP**), `stock-status.test.ts` (`statusOf`), `country-currency.test.ts` (`suggestedCurrencyForCountry`). **HR/Stock/Users (domaines nommés) + POS + devise + CNSS/IRPP = couverts.** Au passage : `payrollBreakdown` extrait comme **source unique** (bulletin PDF + modale + table paie) → fini la divergence d'arrondi entre vues. Reste possible (non fait, **L**) : logique métier **backend** au-delà des helpers déjà testés (`computeReport`/`decideWsAuth`/payroll).
