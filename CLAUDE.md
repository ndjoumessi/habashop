...
### 🟡 Important
- **2 pages > 600L à découper** (Suppliers 619L, Payroll 601L ; POS désormais 495L, HR déjà 555L). Pattern : extraire les modals + sous-composants comme déjà fait pour Stock/HR/Customers/**Orders**/**Users**/**Expenses**/**Planning**/**POS** (Users 821→219L + `components/users/` 7 fichiers ; Expenses 739→307L + `components/expenses/` 7 fichiers ; Planning 708→190L + `components/planning/` 7 fichiers ; POS 627→495L — déjà découpé, extraction écran caisse-fermée + `posTicket.ts` + `pos.anchor.test.tsx` 4 cas via stubs, voir Sprint 9). **L par page**
