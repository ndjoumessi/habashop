...
- **Intégrité prix — SERVEUR-autoritaire** ⚠️ (`sales.ts`) 📖 *raisonnement complet, expositions mesurées et justification des bornes : `docs/lessons/integrite-prix-pos.md`* :
  ...
  - ⚠️ **La protection anti-fraude est la TRACE, pas la branche « offline »** : `clientCreatedAt` n'est renseigné par AUCUN client (mesuré 2026-07-25) ⇒ `honorClientPrice` est **toujours `false` en prod** et cette branche est **du code mort tel que déployé** — et falsifiable de toute façon. Toute divergence écrit `SaleItem.submittedPrice`/`catalogPrice` + `Sale.priceDivergence=true` (audit a posteriori, `GET /api/sales?priceDivergence=true`).
  ...
  - **UI d'audit** (ADMIN seul, `canAuditPrices`, historique POS) : **une source unique `priceGapLevel(rows)`** alimente badge + détail + sous-filtre → `look` ambre (en ligne, inexpliqué = à regarder) · `previous` bleu (le tarif venait de changer) · `offline` gris (montant honoré, bénin). **Biais de PRUDENCE** : une vente mêlant expliqué et inexpliqué reste `look`. Sous-filtre = **« À regarder »**, pas « en ligne » (y garder tous les écarts en ligne y ferait entrer les caches périmés). Vocabulaire **factuel** — « écart de prix », jamais « suspect »/« fraude ».
  - Verrous : `salesTariffIntention.test.ts` (11, **4 sabotages**) · `salesPriceIntegrity.test.ts` (rejoue la requête forgée) · `staleCatalogDivergence.test.ts` (10, **3 sabotages**) · front `cartTariff.test.ts` / `saleReconcile.test.ts` (**2 sabotages**) / `priceGapLevel.test.ts` (11, sabotage vérifié).
...
### 🔴 Critique (suite)
- **Rejeu hors-ligne MOBILE = (b) silencieux** ⚠️ *(mesuré 2026-07-25, surface MOBILE à cadrer — non traitée)* : `mobile/src/hooks/useOfflineSync.ts:37` fait `await salesApi.create(action.payload)` et **jette la réponse**. Comme `clientCreatedAt` n'est jamais renseigné, le serveur **re-tarife** au rejeu : la vente enregistrée peut différer du montant réellement encaissé au comptoir, **sans que personne ne soit prévenu** — exactement le trou que `reconcileSaleTotal` a fermé côté web, encore ouvert côté mobile. Le rejeu se produisant hors de la présence du client, la réconciliation ne peut pas être un toast : il faut une **entrée durable** (historique / file de reprise). **M**
