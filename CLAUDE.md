...
- **Tests de logique métier** : ⏳ **bien entamé** (`3b3f038e` + `bb453ab2`, **171 front** / 145 back). Lots 1+2 sur des fonctions **pures déjà exportées** (zéro extraction) : `pricing.test.ts` (paliers POS `resolveTierPrice`/`validatePriceTiers`), `rbac.test.ts` (`canAccess`/`getLandingForRole`), `payroll-calc.test.ts` (`calcNet`/`calcBrut`/pénalité absence), `stock-status.test.ts` (`statusOf`), `country-currency.test.ts` (`suggestedCurrencyForCountry`). **HR/Stock/Users (les 3 domaines nommés) + POS + devise = couverts.** Reste possible (non fait, **L**) : CNSS/IR détaillé (inline dans `printBulletin` → extraire un helper pur d'abord), logique métier **backend** au-delà des helpers déjà testés (`computeReport`/`decideWsAuth`/payroll).
