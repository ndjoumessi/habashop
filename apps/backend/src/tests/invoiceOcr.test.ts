...
// Le garde de dépense est traversé (boutique cliente saine) : on teste l'OCR, pas la garde.
...
describe('analyzeInvoice — parsing', () => {
  it('parse une réponse JSON valide', async () => {
    ...
    const result = await analyzeInvoice('T', Buffer.from('fake-image'), 'image/jpeg')
  ...
  it('strip les balises markdown autour du JSON', async () => {
    ...
    const result = await analyzeInvoice('T', Buffer.from('fake-image'), 'image/png')
    expect(result.supplierName).toBe('Diallo & Frères')
    expect(result.error).toBeUndefined()
  })
...
describe('analyzeInvoice — erreurs réseau', () => {
  it('propage l\'erreur Anthropic', async () => {
    ...
    await expect(analyzeInvoice('T', Buffer.from('img'), 'image/jpeg')).rejects.toThrow('Network timeout')
  })

  it('lance si ANTHROPIC_API_KEY est absente', async () => {
    delete process.env.ANTHROPIC_API_KEY
    await expect(analyzeInvoice('T', Buffer.from('img'), 'image/jpeg')).rejects.toThrow('ANTHROPIC_API_KEY')
  })
})
...
describe('analyzeInvoice — taille fichier', () => {
  it('traite un buffer de taille normale (< 10MB)', async () => {
    ...
    const result = await analyzeInvoice('T', buf, 'image/jpeg')
    expect(result.supplierName).toBe('Diallo & Frères')
  })
})
