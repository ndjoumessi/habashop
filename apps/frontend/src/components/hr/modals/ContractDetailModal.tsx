...
export default function ContractDetailModal({ lang, fmt, selectedContract, setShowContractDetailModal, openEditModal }: Props) {
  const { currency: curC } = useConfig()
  const contractBd = payrollDisplay({ baseSalary: Number(selectedContract?.salary) || 0, bonus: 0, overtime: 0, deductions: 0, absences: 0 }, curC)
      ...
      <div ref={boxRef} style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:20, width:'100%', maxWidth:480, boxShadow:'var(--sh-xl)', overflow:'hidden' }}>
        <div style={{ padding:'24px 24px 20px', background:`linear-gradient(135deg,${selectedContract.color}18,${selectedContract.color}05)`, borderBottom:'1px solid var(--border)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ width:56, height:56, borderRadius:16, overflow:'hidden', background:`linear-gradient(135deg,${selectedContract.color},${selectedContract.color}88)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'var(--fs-lg)', fontWeight:'var(--fw-semibold)', color:'#fff', flexShrink:0, boxShadow:`0 6px 20px ${selectedContract.color}50` }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:'var(--fs-label)', color:'var(--text3)', marginTop:3 }}>{roleLabel(selectedContract.role, lang)} · {deptLabel(selectedContract.dept, lang)}</div>
            </div>
            <button onClick={()=>setShowContractDetailModal(false)} style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:10, width:32, height:32, cursor:'pointer', color:'var(--text3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'var(--fs-md)' }}>✕</button>
          </div>
        </div>
        <div style={{ padding:24, display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ display:'flex', gap:8 }}>
            <span style={{ fontSize:'var(--fs-label)', fontWeight:'var(--fw-semibold)', padding:'5px 14px', borderRadius:20, background:selectedContract.type==='CDI'?'rgba(108,71,255,.15)':'rgba(14,196,126,.12)', color:selectedContract.type==='CDI'?'var(--p2)':'var(--acc2)' }}>{selectedContract.type}</span>
            <span style={{ fontSize:'var(--fs-label)', fontWeight:'var(--fw-semibold)', padding:'5px 14px', borderRadius:20, background:selectedContract.active?'var(--c-green-bg)':'var(--bg3)', color:selectedContract.active?'var(--acc2)':'var(--text3)' }}>
          ...
          <div style={{ background:'var(--bg3)', borderRadius:12, padding:16, display:'flex', flexDirection:'column', gap:10 }}>
            {[
              ...
              <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontSize:'var(--fs-label)', color:'var(--text3)' }}>{row.label}</span>
                <span style={{ fontSize:'var(--fs-sm)', fontWeight:'var(--fw-semibold)', color:'var(--text)', fontFamily:'var(--mono)' }}>{row.value}</span>
              </div>
            ))}
          </div>
          <div style={{ background:'var(--bg3)', borderRadius:12, padding:16 }}>
            <div style={{ fontSize:'var(--fs-caption)', fontWeight:'var(--fw-bold)', textTransform:'uppercase', letterSpacing:'.8px', color:'var(--text3)', marginBottom:12 }}>💰 {lang === 'en' ? 'COMPENSATION' : lang === 'es' ? 'REMUNERACIÓN' : lang === 'it' ? 'RETRIBUZIONE' : 'RÉMUNÉRATION'}</div>
            {[
              ...
              <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid var(--border)' }}>
                <span style={{ fontSize:'var(--fs-label)', color:'var(--text3)' }}>{row.label}</span>
                <span style={{ fontSize:'var(--fs-sm)', fontWeight:'var(--fw-semibold)', color:row.color, fontFamily:'var(--mono)' }}>{row.value}</span>
              </div>
            ))}
          </div>
        </div>
