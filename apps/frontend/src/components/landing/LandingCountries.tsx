    ...
    <>
      <section aria-label={i('Pays disponibles', 'Available countries', 'Países disponibles', 'Paesi disponibili')} style={{ padding: '72px clamp(16px,4vw,80px)', background: D.bg, borderTop: `1px solid ${D.border}` }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 'var(--fs-title)', color: D.text2, maxWidth: 720, margin: '0 auto 36px', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10 }}>
            {([
              ...
              <span key={name} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 16px', background: D.bg2, border: `1px solid ${D.border}`, borderRadius: 99, fontSize: 'var(--fs-body)', fontWeight: 600, color: D.text2 }}>
                <span style={{ fontSize: 'var(--fs-lg)' }}>{flag}</span>{name}
              </span>
            ))}
            <span style={{ display: 'inline-flex', alignItems: 'center', padding: '9px 16px', background: 'rgba(108,71,255,.1)', border: '1px solid rgba(108,71,255,.25)', borderRadius: 99, fontSize: 'var(--fs-body)', fontWeight: 700, color: D.p3 }}>
