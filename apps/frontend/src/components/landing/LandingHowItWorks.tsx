          ...
          {steps.map(s => (
            <div key={s.num} style={{ textAlign: 'center', padding: '12px 8px' }}>
              <div style={{
                ...
                color: '#fff', fontSize: 'var(--fs-2xl)', fontWeight: 900,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: MONO,
                boxShadow: '0 8px 24px rgba(108,71,255,.35)',
              }}>{s.num}</div>
              <div style={{ fontSize: 'var(--fs-title)', fontWeight: 800, color: D.text, marginBottom: 8 }}>{s.title}</div>
              <div style={{ fontSize: 'var(--fs-sm)', color: D.text2, lineHeight: 1.65 }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </section>
