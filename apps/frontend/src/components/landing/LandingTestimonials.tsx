        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(290px,1fr))', gap: 16, maxWidth: 1060, margin: '0 auto' }}>
          {testimonials.map(t => (
            ...
            >
              ...
              <p style={{ fontSize: 'var(--fs-body)', lineHeight: 1.7, color: D.text, marginBottom: 18, fontStyle: 'italic' }}>"{t.quote}"</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 38, height: 38, borderRadius: '50%', background: t.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-sm)', fontWeight: 900, color: t.fg, flexShrink: 0, boxShadow: `0 4px 14px ${t.color}55` }}>
                  {t.avatar}
                </div>
                <div>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 800, color: D.text }}>{t.name}</div>
                  <div style={{ fontSize: 'var(--fs-caption)', color: D.text3 }}>{t.role}</div>
                </div>
              </div>
            </div>
          ))}
