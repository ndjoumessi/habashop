...
- **Lot 4 — tailles/graisses inline → tokens** : 245 fontSize inline < 11 → **11 (plancher)** ; 450 fontWeight 500/700/800 → `var(--fw-*)` (identique). Déférés : cap 600/900 (change le poids), tokenisation masse padding/fontSize on-scale (churn invisible).
- **Lot 5 — tooltips souris-only → `<FocusTooltip>`** : **rien à faire** — les seuls tooltips custom souris-only (badges devise/langue, cursor-tracking) ont été migrés en Vague 2. Restants = `title=` natifs (browser, couplés à `aria-label` sur boutons focusables) → pas « souris-only custom », non convertis (convertir changerait l'ordre de tabulation sans gain lecteur d'écran).
