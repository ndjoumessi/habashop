...
describe('les points d’appel déclarent la BONNE politique', () => {
  ...
  it("POS.tsx (MTN) exige 'international' — le bac à sable MTN utilise des numéros étrangers", () => {
    const src = read(resolve(__dirname, '..', 'pages', 'POS.tsx'))
    ...
    expect(calls[0]).toContain("'international'")
    expect(calls[0]).not.toContain("'cm-only'")
  })
})
