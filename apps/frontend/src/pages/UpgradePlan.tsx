    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#07070F,#0D0D1C)', fontFamily: 'var(--font)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 520, display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 'var(--fw-semibold)', color: '#F0F0FF' }}>⚡ {i('Passer au Pro', 'Upgrade to Pro', 'Pasarse a Pro', 'Passa a Pro')}</div>
        </div>
        <div style={{ width: 70 }} />
      </div>
      ...
      <div style={{ width: '100%', maxWidth: 520, background: 'linear-gradient(160deg,#0D0D1C,#111128)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 24, overflow: 'hidden' }}>
        ...
        {step === 'plan' && (
          <div style={{ padding: 32 }}>
            <h2 style={{ fontSize: 20, fontWeight: 'var(--fw-semibold)', color: '#F0F0FF', marginBottom: 6 }}>{i('Choisissez votre plan', 'Choose your plan', 'Elige tu plan', 'Scegli il tuo piano')}</h2>
              ...
              {['pro', 'enterprise'].map(planId => {
                ...
                return (
                  <button key={planId} type="button" onClick={() => set({ plan: planId })} style={{ padding: 16, borderRadius: 14, background: isSelected ? (planId === 'pro' ? 'rgba(108,71,255,.12)' : 'rgba(255,149,0,.1)') : 'rgba(255,255,255,.03)', border: `2px solid ${isSelected ? (planId === 'pro' ? 'rgba(108,71,255,.4)' : 'rgba(255,149,0,.35)') : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--font)', transition: 'all .2s', display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 15, fontWeight: 'var(--fw-semibold)', color: isSelected ? (planId === 'pro' ? 'var(--p3)' : 'var(--acc)') : 'var(--text)', marginBottom: 2 }}>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontSize: 20, fontWeight: 'var(--fw-semibold)', color: isSelected ? (planId === 'pro' ? 'var(--p3)' : 'var(--acc)') : 'var(--text2)', fontFamily: 'var(--mono)' }}>{price.toLocaleString('fr-FR')}</div>
                      <div style={{ fontSize: 10, color: 'var(--text3)' }}>F/{i('mois', 'mo', 'mes', 'mese')}</div>
                    </div>
                    {isSelected && <div style={{ width: 22, height: 22, borderRadius: '50%', background: planId === 'pro' ? 'var(--p2)' : 'var(--acc)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', fontWeight: 'var(--fw-regular)', flexShrink: 0 }}>✓</div>}
                  </button>
                )
              })}
            </div>
        ...
        {step === 'payment' && (
          <div style={{ padding: 32 }}>
            <h2 style={{ fontSize: 20, fontWeight: 'var(--fw-semibold)', color: '#F0F0FF', marginBottom: 6 }}>💳 {i('Mode de paiement', 'Payment method', 'Método de pago', 'Metodo di pagamento')}</h2>
            ...
            <div style={{ padding: '12px 16px', marginBottom: 20, background: 'rgba(108,71,255,.08)', border: '1px solid rgba(108,71,255,.2)', borderRadius: 12, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 'var(--fw-semibold)', color: 'var(--p3)', fontFamily: 'var(--mono)' }}>{amount.toLocaleString('fr-FR')} F CFA</span>
            </div>
              ...
              {PAYMENT_METHODS.map(m => {
                ...
                return (
                  <button key={m.id} type="button" onClick={() => set({ paymentMethod: m.id })} style={{ padding: '12px 16px', borderRadius: 12, background: isSelected ? `${m.color}15` : 'rgba(255,255,255,.03)', border: `1.5px solid ${isSelected ? m.color + '44' : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--font)', transition: 'all .15s', display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 'var(--fw-semibold)', color: isSelected ? m.color : 'var(--text)' }}>{m.name}</div>
            ...
            {isAuto ? (
              ...
              <>
                <div style={{ padding: '14px 16px', marginBottom: 16, background: 'rgba(255,184,0,.06)', border: '1px solid rgba(255,184,0,.15)', borderRadius: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 'var(--fw-regular)', color: 'var(--warn)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '.5px' }}>📋 {i('Instructions de paiement', 'Payment instructions', 'Instrucciones de pago', 'Istruzioni di pagamento')}</div>
                  {(() => {
        ...
        {step === 'done' && (
          <div style={{ padding: '48px 32px', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 22, fontWeight: 'var(--fw-semibold)', color: '#F0F0FF', marginBottom: 10 }}>{i('✅ Demande envoyée !', '✅ Request sent!', '✅ ¡Solicitud enviada!', '✅ Richiesta inviata!')}</h2>
