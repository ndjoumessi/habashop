  ...
  return (
    <div className="public-scope" style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 0%, rgba(124,58,237,.16), transparent 55%), linear-gradient(135deg,#0F0A2E 0%,#0A0A0F 58%,#0F0F1A 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'var(--font)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
        ...
        <span style={{ fontSize: 'var(--fs-2xl)', fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.5px' }}>Haba<span className="gold-text">Shop</span></span>
      </div>

      <div style={{ width: '100%', maxWidth: 460 }}>
        <h1 style={{ fontSize: 'var(--fs-2xl)', fontWeight: 900, color: 'var(--public-text)', textAlign: 'center', marginBottom: 6, letterSpacing: '-.3px' }}>
          {i('Choisissez une boutique', 'Choose a shop', 'Elige una tienda', 'Scegli un negozio')}
        </h1>
        <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--text3)', textAlign: 'center', marginBottom: 24 }}>
        ...
        {error && (
          <div role="alert" style={{ background: 'rgba(255,59,92,.1)', border: '1px solid rgba(255,59,92,.3)', borderRadius: 12, padding: '10px 14px', fontSize: 'var(--fs-sm)', color: '#FF3B5C', marginBottom: 16 }}>{error}</div>
        )}
        ...
        <button type="button" onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, width: '100%', marginTop: 22, padding: 10, background: 'none', border: 'none', color: 'var(--text3)', fontSize: 'var(--fs-sm)', cursor: 'pointer', fontFamily: 'var(--font)' }}>
