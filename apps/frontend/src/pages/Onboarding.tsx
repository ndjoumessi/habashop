  ...
  return (
    <div className="public-scope" style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 0%, rgba(124,58,237,.16), transparent 55%), linear-gradient(135deg,#0F0A2E 0%,#0A0A0F 58%,#0F0F1A 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'var(--font)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32 }}>
        <div style={{ width: 44, height: 44, borderRadius: 14, background: 'linear-gradient(135deg,#7C3AED,#8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, boxShadow: '0 8px 24px rgba(124,58,237,.4)' }}>🛒</div>
      ...
      <div style={{ width: '100%', maxWidth: 560, marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--public-gold)', fontWeight: 700 }}>{Math.round((step / STEPS.length) * 100)}%</span>
        </div>
        <div style={{ height: 4, background: 'rgba(255,255,255,.08)', borderRadius: 99, overflow: 'hidden' }}>
          <div style={{ width: `${(step / STEPS.length) * 100}%`, height: '100%', background: 'linear-gradient(90deg,#7C3AED,#8B5CF6)', borderRadius: 99, transition: 'width .4s cubic-bezier(.34,1.56,.64,1)', boxShadow: '0 0 12px rgba(124,58,237,.5)' }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          {STEPS.map(s => (
            <div key={s.id} style={{ width: 28, height: 28, borderRadius: '50%', background: step > s.id ? 'var(--acc2)' : step === s.id ? 'linear-gradient(135deg,#7C3AED,#8B5CF6)' : 'rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: step > s.id ? 12 : 14, border: step === s.id ? '2px solid rgba(124,58,237,.5)' : 'none', transition: 'all .3s' }}>{step > s.id ? '✓' : s.icon}</div>
          ))}
        </div>
      </div>

      {/* Card */}
      <div style={{ width: '100%', maxWidth: 560, background: 'linear-gradient(160deg,#0F0F1A,#0A0A0F)', border: '1px solid var(--public-border)', borderRadius: 24, overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,.6)' }}>

        {step === 1 && (
          <div style={{ padding: 36 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.8px', color: 'var(--public-gold)', marginBottom: 8 }}>{i('BIENVENUE', 'WELCOME', 'BIENVENIDO', 'BENVENUTO')}</div>
            <h1 style={{ fontSize: 26, fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.5px', marginBottom: 6, lineHeight: 1.2 }}>{i('🎉 Créons votre boutique', "🎉 Let's set up your shop", '🎉 Creemos tu tienda', '🎉 Creiamo il tuo negozio')}</h1>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {[
                  ...
                  <button key={t.v} type="button" onClick={() => set({ shopType: t.v })} style={{ padding: '12px 14px', borderRadius: 12, background: form.shopType === t.v ? 'rgba(124,58,237,.15)' : 'rgba(255,255,255,.04)', border: `1.5px solid ${form.shopType === t.v ? 'rgba(124,58,237,.4)' : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--font)', transition: 'all .15s' }}>
                    <div style={{ fontSize: 20, marginBottom: 4 }}>{t.icon}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: form.shopType === t.v ? 'var(--public-primary-2)' : 'var(--text2)' }}>{t.label[lang as keyof typeof t.label] ?? t.label.fr}</div>
                  </button>
                ))}
              </div>
            </div>
        ...
        {step === 2 && (
          <div style={{ padding: 36 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.8px', color: 'var(--public-gold)', marginBottom: 8 }}>{i('LOCALISATION', 'LOCATION', 'UBICACIÓN', 'POSIZIONE')}</div>
            <h2 style={{ fontSize: 22, fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.3px', marginBottom: 6 }}>📍 {i('Où êtes-vous situé ?', 'Where are you located?', '¿Dónde estás ubicado?', 'Dove sei situato?')}</h2>
        ...
        {step === 3 && (
          <div style={{ padding: 36 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.8px', color: 'var(--public-gold)', marginBottom: 8 }}>{i('CONFIGURATION', 'CONFIGURATION', 'CONFIGURACIÓN', 'CONFIGURAZIONE')}</div>
            <h2 style={{ fontSize: 22, fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.3px', marginBottom: 6 }}>⚙️ {i('Paramètres de base', 'Basic settings', 'Ajustes básicos', 'Impostazioni base')}</h2>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                {([['XOF', '🇸🇳', 'CFA Ouest'], ['XAF', '🇨🇲', 'CFA Centre'], ['EUR', '🇪🇺', 'Euro'], ['USD', '🇺🇸', 'Dollar US'], ['CAD', '🇨🇦', 'Dollar CA'], ['GBP', '🇬🇧', 'Livre £']] as const).map(([code, flag, name]) => (
                  <button key={code} type="button" onClick={() => { setCurrencyTouched(true); set({ currency: code as Currency }) }} style={{ padding: '10px 8px', borderRadius: 10, background: form.currency === code ? 'rgba(234,179,8,.14)' : 'rgba(255,255,255,.04)', border: `1.5px solid ${form.currency === code ? 'rgba(234,179,8,.45)' : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', textAlign: 'center', fontFamily: 'var(--font)', transition: 'all .15s' }}>
                    <div style={{ fontSize: 20 }}>{flag}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: form.currency === code ? 'var(--public-gold)' : 'var(--text2)', marginTop: 3 }}>{code}</div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
                {([['fr', '🇫🇷', 'FR'], ['en', '🇬🇧', 'EN'], ['es', '🇪🇸', 'ES'], ['it', '🇮🇹', 'IT']] as const).map(([code, flag, name]) => (
                  <button key={code} type="button" onClick={() => set({ language: code as Lang })} style={{ padding: '10px 6px', borderRadius: 10, background: form.language === code ? 'rgba(124,58,237,.15)' : 'rgba(255,255,255,.04)', border: `1.5px solid ${form.language === code ? 'rgba(124,58,237,.4)' : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', textAlign: 'center', fontFamily: 'var(--font)', transition: 'all .15s' }}>
                    <div style={{ fontSize: 20 }}>{flag}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: form.language === code ? 'var(--public-primary-2)' : 'var(--text2)' }}>{name}</div>
                  </button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {[0, 10, 18, 20, 25].map(r => (
                  <button key={r} type="button" onClick={() => set({ taxRate: r })} style={{ padding: '8px 16px', borderRadius: 10, background: form.taxRate === r ? 'rgba(124,58,237,.15)' : 'rgba(255,255,255,.04)', border: `1px solid ${form.taxRate === r ? 'rgba(124,58,237,.4)' : 'rgba(255,255,255,.08)'}`, cursor: 'pointer', color: form.taxRate === r ? 'var(--public-primary-2)' : 'var(--text3)', fontFamily: 'var(--font)', fontSize: 13, fontWeight: 700, transition: 'all .15s' }}>{r}%</button>
                ))}
        ...
        {step === 4 && (
          <div style={{ padding: 36 }}>
            <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.8px', color: 'var(--public-gold)', marginBottom: 8 }}>{i('PREMIER PRODUIT', 'FIRST PRODUCT', 'PRIMER PRODUCTO', 'PRIMO PRODOTTO')}</div>
            <h2 style={{ fontSize: 22, fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.3px', marginBottom: 6 }}>📦 {i('Ajoutez votre premier produit', 'Add your first product', 'Agrega tu primer producto', 'Aggiungi il tuo primo prodotto')}</h2>
        ...
        {step === 5 && (
          <div style={{ padding: '48px 36px', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 26, fontWeight: 900, color: 'var(--public-text)', letterSpacing: '-.5px', marginBottom: 10 }}>{i('🎉 Votre boutique est prête !', '🎉 Your shop is ready!', '🎉 ¡Tu tienda está lista!', '🎉 Il tuo negozio è pronto!')}</h2>
