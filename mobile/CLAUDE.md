...
- ⚠️ **« OTA » ci-dessus = l'APPAREIL DE TEST, canal `preview`. Le canal `production` NE LIVRE RIEN — et il n'y a personne à qui livrer** *(mesuré 2026-07-31, recompté 2026-08-06)* : le seul build store (`1f6bf56f-…`) est en **runtime 1.2.0**, **aucune fiche store publiée** (Play 404, App Store 0 résultat) et **1 seule installation** en base — l'appareil de test, sur `demo-tenant-001`, et `eas channel:view production` rend **« No branches are pointed to this channel »** — les 3 update groups de la branche `production` sont en 1.4.3 et n'ont donc **jamais rien livré**. Ne pas transposer le swap 1.4.3 à la prod : `main` a franchi des **ruptures natives** depuis 1.2.0 (Sentry, `expo-background-fetch`→`expo-background-task`, plugin `withLocalizedPermissions`), qu'une OTA ne porte pas → **crash au lancement**, le jour où un parc existera. Suivi : **#187** (pipeline OTA) · **#188** (release native 1.5.0).
- ⚠️ **AVANT toute OTA, prouver la portabilité par l'EMPREINTE, pas par lecture de diff** : `eas fingerprint:compare --build-id <id-du-build-cible>` — il liste les chemins à dépendance native et le delta d'autolinking. *(Flag `--build-id`, pas `--buildId` ; et `eas build:view` **comme `eas update:view`** REFUSENT `--non-interactive` — « Nonexistent flag », vérifié le 2026-08-12.)*
...
- **Validé device (2026-08-13, OTA canal preview, runtime 1.4.3) :** lignes de panier non enroulées ✅, grille de caisse — dernière tuile NON étirée ✅, kiosque — noms de produits entiers et total non tronqué ✅. *(Les TROIS défauts de mise en page du jour, tous trouvés à l'œil et invisibles en jest.)* *(Mesuré d'abord sur émulateur via Expo Go, puis confirmé par Nelson sur l'appareil.)*
