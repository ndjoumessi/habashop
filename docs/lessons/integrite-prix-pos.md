...
> 🛑 **À LIRE AVANT LA SUITE — deux passages ci-dessous sont PÉRIMÉS depuis l'option A (2026-08).**
> Cette page a été **complétée par ajout**, pas réécrite : la mise à jour est en bas, et *un ajout
> n'annule pas ce qu'il contredit* — on lit depuis le haut.
>
> 1. La branche « offline honoré » y est décrite comme **du code mort tel que déployé**. Exact au
>    2026-07-25, **faux depuis l'option A** : elle est vivante, `clientCreatedAt` /
>    `REPLAY_THRESHOLD_MS` / `honorClientPrice` ont été SUPPRIMÉS et remplacés par
>    `offlineReplay && staleCatalogAt !== null`.
> 2. Le modèle d'audit y compte **TROIS** niveaux et classe un écart honoré en **gris « bénin »**.
>    Il y en a **QUATRE**, et `honored` est **AMBRE « à vérifier »** — parce que **l'argent a
>    bougé**. ⚠️ `honored` ne doit JAMAIS retomber dans le bleu ni dans le gris : là-bas le
>    serveur a corrigé, donc l'argent est juste ; ici il y a une caisse à vérifier.
>
> Les corrections sont appliquées en place ci-dessous. Le texte d'origine est conservé pour le
> raisonnement, pas pour la règle : **la règle vit dans `CLAUDE.md` § POS / Ventes.**
