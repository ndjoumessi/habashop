...
- **Photos produit — stockage objet (Cloudflare R2)** ⚠️ *(2026-08-12 — stockage **et envoi WEB** livrés ; **mobile PAS encore**)*
  ...
  - Verrou : `productPhotoField.test.ts` (8, **3 sabotages**, dont la collision émoji/photo jouée sur le DOM rendu). ⚠️ `resizeToBlob` n'est **pas** exerçable sous jsdom (ni canvas ni décodeur) : il est simulé, et ce fichier garde la DÉCISION, jamais les pixels.
